...
def read(path, config):
    ...
    try:
        ...
        for line in open(path):
            ...
            for option in ("socket", "noblock", "device", "user", "password", "ssl-ca-cert", "ssl-no-verify-hostname", "ssl-no-verify-certificate", "brobox", "fleet", "uid", "mfa", "bearer-token", "no-password-save", "automation", "timeout", "retries", "retry-max-time", "error-format"):
                if k.lower() == option:
                    config[option] = v
